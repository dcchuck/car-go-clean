...
The attestation entry above is a commit object, not a movable tag ref or
annotated-tag object. Rehearsal uses the current Node 24 `actions/attest`
implementation directly with `subject-path`; it does not introduce the older
Node 20 provenance wrapper.
